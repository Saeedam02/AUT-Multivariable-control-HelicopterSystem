function [G, meta] = buildHelicopterPlant(varargin)
%BUILDHELICOPTERPLANT Construct the 10-output, 4-input helicopter plant.
%
%   G = buildHelicopterPlant() builds the plant exactly as it appears in the
%   original repository scripts. The model is continuous-time and represented
%   as a 10-by-4 transfer-function matrix.
%
%   [G,META] = buildHelicopterPlant("Variant","legacy") returns metadata
%   describing the model source and the known transcription anomaly in G(3,2).
%
%   [G,META] = buildHelicopterPlant("Variant","consistent-denominator") uses
%   8733 as the s^3 coefficient in the G(3,2) denominator, matching the common
%   eighth-order denominator used by nearly all neighboring channels.
%
% IMPORTANT
% ---------
% The original source uses 87338 in G(3,2), while the corresponding common
% denominator elsewhere uses 8733. The repository does not contain enough
% provenance to prove which value is physically correct. Therefore the default
% remains "legacy" for scientific traceability. Use the alternative variant
% only after checking the original model-identification/linearization source.
%
% The original repository does not document the physical names or units of the
% four inputs and ten outputs. Generic names u1..u4 and y1..y10 are therefore
% assigned rather than inventing physical meanings.
%
% Requires: Control System Toolbox.

    parser = inputParser;
    parser.FunctionName = mfilename;
    addParameter(parser, "Variant", "legacy", @(x) any(strcmpi(string(x), ...
        ["legacy", "consistent-denominator"])));
    addParameter(parser, "MinrealTolerance", 0, @(x) isnumeric(x) && isscalar(x) && x >= 0);
    parse(parser, varargin{:});

    variant = lower(string(parser.Results.Variant));
    tol = parser.Results.MinrealTolerance;

    % A transfer-function variable keeps the original equations readable while
    % avoiding the Symbolic Math Toolbox dependency used in the legacy script.
    s = tf('s');
    ...
    G3_2 =(-870.2*s^6 - 1.71e04*s^5 - 1.467e05*s^4 - 8.811e04*s^3 - 2757*s^2 - 1.134e04*s - 7.263e-13)/(s^8 + 27.2*s^7 + 430.8*s^6 + 3315*s^5 + 1.426e04*s^4 + g32_s3_coeff*s^3 + 5415*s^2 + 208.2*s + 2.402);
    ...
    % Assemble rows = measured outputs and columns = manipulated inputs.
    ...
    G.InputName = cellstr(compose("u%d", 1:4));
    G.OutputName = cellstr(compose("y%d", 1:10));
    G.Name = "AUT helicopter MIMO plant";

    meta = struct();
    meta.variant = variant;
    meta.size = size(G);
    meta.sourceFile = "legacy/MFILE_Design.m";
    meta.sourceRepository = "Saeedam02/AUT-Multivariable-control-HelicopterSystem";
    meta.channelLabelsArePlaceholders = true;
    meta.inputNames = string(G.InputName);
    meta.outputNames = string(G.OutputName);
    meta.g32DenominatorS3Coefficient = g32_s3_coeff;
    meta.knownAnomaly = struct( ...
        "channel", "G(3,2)", ...
        "legacyValue", 87338, ...
        "neighboringCommonValue", 8733, ...
        "verifiedPhysicalValue", NaN, ...
        "status", "requires verification against original model source");
end
