...
%[A_P,B_P,C_P,D_P]=linmod('siso');

%P=ss(A_P,B_P,C_P,D_P);
%Iz=[1:2]';     %Output of unSertaintly
%Iv=[1:2]';     %Input of unSertaintly
%Ie=[3:4]';     %Output of Error Signal
%Iw=[3,4]';     %Input of System
%Iy=[5,6]';     %Output of Controller Signal 
%Iu=[5]';       %Input of Signal_Control

%ny=2;          %number of Output
%nu=1;          %number of Control_input
%ne=2;          %number of Output_error
%nw=2;          %number of Input_Ref

%Pnominal=P([Ie;Iy],[Iw;Iu]);   %it Get me Error!

state=input("Choose model__(1_for M_K form) & (2_for Delta_M_K form):");
...
%                                  _______          ________
%                                 |       |        |        |
%              _______+( )________|   K   |________|   G(s) |______.______
%                      -|         |_______|        |________|      |
%                       |                                          |
%                       |__________________________________________|
%
%     And We that the Model:
%                            __________
%              w  _________>|          |_________> y
%                      ____>|    M     |_____
%                     |     |__________|     |
%                     |                      |
%                     |                      |
...
VALUE_DELTA=I - M22 * Ks;
DELTA=pinv(VALUE_DELTA);

LLFT=M11 + M12*Ks*DELTA*M21
end
%% K_M_Delta form of system
if(state==2)
%    Block Diagram:
%                                            ______     _______
%                                     ______|      |___|       |___
...
%                          _______   |                             |
%                         |       |  |                            +|
%      _______+( )________|   K   |__.___________________________+( )__|  G(s) |____( )__.______
%              -|         |_______|                                    |_______|         |
%               |                                                                        |
...
K=k*eye(c,c);
I=eye(r,c);
N11=p11+p13*K*pinv(I - p33*K)*p31;
N12=p12+p13*K*pinv(I - p33*K)*p32;
N21=p21+p23*K*pinv(I - p33*K)*p31;
N22=p22+p23*K*pinv(I - p33*K)*p32
N_Mat=[N11 N12;N21 N22];
...
D=d*eye(c,c);
VALUE_DELTA=eye(r,c) - N11 * D;
DELTA=pinv(VALUE_DELTA);

ULFT=N22 + N21*D*DELTA*M21
end
